...
source('scripts/create_long_data.R', encoding = 'UTF-8')
source('scripts/create_follow_up_plot.R', encoding = 'UTF-8')
```

Les participants·e·s de l'étude ont été suivis entre `r min(.followup_time$followup)` et `r max(.followup_time$followup)` jours avec une durée médiane de `r median(.followup_time$followup)` jours. En termes de consultation de suivi, `r nrow(.followup_time)` patient·e·s ont été suivi·e·s au moins jusqu'à la première consultation trimestrielle (T1), `r sum(.followup_time$followup > 100)` patient·e·s, jusqu'à la deuxième (T2), `r sum(.followup_time$followup > 229)`, jusqu'à la troisième (T3), et `r sum(.followup_time$followup > 300)`, jusqu'à la quatrième (T4). Un·e seul·e patient·e a été perdu·e de vue, suite à sa décision d'interrompre le THAG. Comme le montre la figure \@ref(fig:FollowPlot), la répartition par genres reste globalement équilibrée. On constate davantage de personnes transféminines jusqu'à T3 inclus. Cela est dû au fait que j'avais un peu plus de patient·e·s transmasculin·e·s quand j'ai commencé à faire des suivis hormonaux, ce qui fut vite contrebalancé.

```{r FollowPlot, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.cap="Nombre de patient·e·s par consultation trimestrielle et par genre", fig.dim=c(10,8), out.width = '100%', eval = knitr::is_html_output()}
...
Toujours dans le but d'appréhender au mieux les besoins des patient·e·s, de les informer au mieux, et de les référer vers les spécialistes adéquat·e·s, je leur ai demandé s'iels identifient déjà à ce stade des chirurgies nécessaires pour les rapprocher de leurs points de confort.

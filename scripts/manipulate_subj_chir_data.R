...
.chir_data_F <- dplyr::mutate(.subj_chir_data_F, N_chir = ifelse(.subj_chir_data_F$bes_mam == 2, 1, 0) +
...
.p_vag2 <- percent(sum(.chir_data_F$bes_vag == '2', na.rm = T)/nrow(.chir_data_F))
.p_vag1 <- percent(sum(.chir_data_F$bes_vag == '1', na.rm = T)/nrow(.chir_data_F))
.p_ffs2 <- percent(sum(.chir_data_F$bes_ffs == '2', na.rm = T)/nrow(.chir_data_F))
...
tbl_summary(.chir_data_M)

.N_tot_attentes_chir_M <- nrow(.chir_data_M)
.p_torso2 <- percent(sum(.chir_data_M$bes_torso == '2', na.rm = T)/.N_tot_attentes_chir_M)
.p_torso1 <- percent(sum(.chir_data_M$bes_torso == '1', na.rm = T)/.N_tot_attentes_chir_M)
.p_hystero <- percent(sum(.chir_data_M$bes_hystero == '2', na.rm = T)/.N_tot_attentes_chir_M)
